...
class SearchAnalyticsApp:
    def __init__(self, root):
        ...
        self.using_fake_data = False
        self.fake_data = {
            'total_users': 1089,
            'daily_users': 127,
            'total_searches': 10876,
            'actor_searches': 670,
            'today_searches': 243
        }
    ...
    def create_header(self, parent):
        ...
        # Toggle Fake Data button
        self.fake_data_btn = ttk.Button(
            button_frame,
            text="Load Sample Data",
            command=self.toggle_fake_data
        )
        self.fake_data_btn.pack(side=tk.RIGHT, padx=5)
    ...
    def toggle_fake_data(self):
        self.using_fake_data = not self.using_fake_data

        if self.using_fake_data:
            self.fake_data_btn.configure(text="Reset to Real Data")
            self.generate_fake_search_data()
        else:
            self.fake_data_btn.configure(text="Load Sample Data")

        self.refresh_data()

    def generate_fake_search_data(self):
        # Generate fake recent searches
        self.fake_recent_searches = []
        usernames = ["user123", "searchmaster", "moviebuff", "filmfan", "cinephile"]
        queries = ["Tom Cruise", "Brad Pitt", "Leonardo DiCaprio", "Morgan Freeman", "Robert De Niro"]

        for _ in range(50):
            timestamp = datetime.now()
            self.fake_recent_searches.append((
                timestamp.strftime('%Y-%m-%d %H:%M'),
                random.randint(1000, 9999),
                random.choice(usernames),
                random.choice(queries),
                "telegram",
                "Actor" if random.random() > 0.5 else "General"
            ))

        # Generate fake top searches
        self.fake_top_searches = [
            ("Tom Cruise", 156),
            ("Brad Pitt", 143),
            ("Leonardo DiCaprio", 128),
            ("Morgan Freeman", 112),
            ("Robert De Niro", 98),
            ("Al Pacino", 87),
            ("Johnny Depp", 76),
    ...
    def update_statistics(self):
        """Update dashboard statistics with either real or fake data"""
        try:
            if self.using_fake_data:
                # Base values for fake data
                base_stats = {
                    'total_users': 1089,
                    'daily_users': 127,
                    'total_searches': 10876,
                ...
                for key, base_value in base_stats.items():
                    # Calculate variation range (0.5% to 2% of base value)
                    variation_range = int(base_value * 0.02)
                    variation = random.randint(-variation_range, variation_range)

                    # Ensure we don't go below 0 for any stat
                    display_value = max(0, base_value + variation)

                    # Format different stats appropriately
                    if key == 'total_users':
                        self.stats_vars[key].set(f"Total Users: {display_value:,}")
                    elif key == 'daily_users':
                        self.stats_vars[key].set(f"Today's Users: {display_value:,}")
                    elif key == 'total_searches':
                        self.stats_vars[key].set(f"Total Searches: {display_value:,}")
                    elif key == 'actor_searches':
                        self.stats_vars[key].set(f"Actor Searches: {display_value:,}")
                    elif key == 'today_searches':
                        self.stats_vars[key].set(f"Today's Searches: {display_value:,}")

            else:
                # Real database statistics
                with database_connection(self.db.db_name) as conn:
                    cursor = conn.cursor()
                    ...
                    cursor.execute('''
                        SELECT COUNT(DISTINCT user_id) 
                        FROM user_searches 
                        WHERE date(search_time, 'localtime') = date('now', 'localtime')
                    ''')
            ...
            logging.error(f"Database error while updating statistics: {db_error}")
            # Don't show error message to user for background updates
            if not getattr(self, 'update_thread', None) or \
                    threading.current_thread() != self.update_thread:
                messagebox.showerror("Database Error",
                                     "Failed to retrieve statistics from database")
        except Exception as e:
            logging.error(f"Unexpected error while updating statistics: {e}")
            if not getattr(self, 'update_thread', None) or \
                    threading.current_thread() != self.update_thread:
                messagebox.showerror("Error",
                                     "Failed to update statistics")
        finally:
            # Ensure stats vars exist even if there was an error
            for key in ['total_users', 'daily_users', 'total_searches',
                        'actor_searches', 'today_searches']:
                if key not in self.stats_vars:
                    self.stats_vars[key] = tk.StringVar(value=f"{key.replace('_', ' ').title()}: 0")
    ...
    def update_period_tree(self, cursor, tree, time_condition):
        if self.using_fake_data:
            ...
            # Insert fake data
            for rank, (query, count) in enumerate(self.fake_top_searches, 1):
                # Add some random variation to the count
                varied_count = count + random.randint(-5, 5)
                tree.insert('', 'end', values=(rank, query, varied_count))
        else:
            # Original update_period_tree code
            query = f'''
...
def main():
    """Improved main function with proper error handling"""
    try:
